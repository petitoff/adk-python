...
class AdkDevServer:
  ...
  def __init__(
    ...
    if session_db_url:
      if session_db_url.startswith("agentengine://"):
        ...
        self.agent_engine_id = session_db_url.split("://")[1]
  ...
  async def get_session(
    ...
    app_name = self.agent_engine_id if self.agent_engine_id else app_name
  ...
  async def agent_run(self, req: AgentRunRequest) -> list[Event]:
    # Connect to managed session if agent_engine_id is set.
    app_name = self.agent_engine_id if self.agent_engine_id else req.app_name
    session = await self.session_service.get_session(
        app_name=app_name, user_id=req.user_id, session_id=req.session_id
    )
    if not session:
