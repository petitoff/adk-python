...
class GcsEvalSetsManager(EvalSetsManager):
  ...
  def _write_eval_set(self, blob_name: str, eval_set: EvalSet):
  ...
  def _load_eval_set_from_blob(self, blob_name: str) -> Optional[EvalSet]:
    """Returns an EvalSet that is read from the given GCS blob name."""
    blob = self.bucket.blob(blob_name)
    if not blob.exists():
      return None
  ...
  def create_eval_set(self, app_name: str, eval_set_id: str):
    ...
    self._write_eval_set(new_eval_set_blob_name, new_eval_set)
  ...
  def add_eval_case(self, app_name: str, eval_set_id: str, eval_case: EvalCase):
    ...
    """
    updated_eval_set = self._add_eval_case_to_eval_set(
        app_name, eval_set_id, eval_case
    )
    eval_set_blob_name = self._get_eval_set_blob_name(app_name, eval_set_id)
    self._write_eval_set(eval_set_blob_name, updated_eval_set)
  ...
  def update_eval_case(
    ...
    """
    updated_eval_set = self._update_eval_case_in_eval_set(
  ...
  def delete_eval_case(
    ...
    """
    updated_eval_set = self._delete_eval_case_from_eval_set(
